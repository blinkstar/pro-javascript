...
随着IE7的发布，其JavaScript引起的垃圾收集例程改变了工作方式：  
...
使用具备垃圾收集机制的语音编写程序，开发人员一般不必操心内存管理的问题。  
...
但是**对于全局变量** globalPerson而言，则需要我们在不使用它的时候手工为它解除引用，这也正是上面例子中最后一行代码的目的。
